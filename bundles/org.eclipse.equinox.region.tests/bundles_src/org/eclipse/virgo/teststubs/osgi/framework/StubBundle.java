...
public final class StubBundle implements Bundle {
    ...
    public URL getEntry(String path) {
        ensureNotUninstalled(); // Replaces AspectJ BundleValid aspect
    ...
    public StubBundle setState(int state) {
        synchronized (this.stateMonitor) {
            this.state = state;
            // Fire appropriate bundle event based on state change (replaces AspectJ BundleEvents aspect)
            switch (state) {
                case INSTALLED:
                    // INSTALLED state can mean bundle was just installed or updated
                    // The calling method will handle firing the correct event
                    break;
                case STARTING:
                ...
                case STOPPING:
                    ...
                    break;
                case RESOLVED:
                    // RESOLVED can mean bundle was stopped, check if coming from STOPPING
                    // The calling method will handle firing STOPPED event if needed
                    break;
                case UNINSTALLED:
    ...
    public void stop(int options) throws BundleException {
        ...
        synchronized (this.stateMonitor) {
            ...
            setState(RESOLVED);
            // Fire STOPPED event after transitioning to RESOLVED (replaces AspectJ BundleEvents aspect)
    ...
    public void uninstall() throws BundleException {
        ...
        synchronized (this.stateMonitor) {
            ...
            // Update lastModified timestamp (replaces AspectJ BundleModifier aspect)
            setLastModified(new java.util.Date().getTime());
        }
    }
    ...
    public void update() throws BundleException {
        ...
        synchronized (this.updateDelegateMonitor) {
            synchronized (this.stateMonitor) {
                ...
                setState(INSTALLED);
                // Fire UPDATED event after setting to INSTALLED (replaces AspectJ BundleEvents aspect)
    ...
    /**
     * Helper method to fire bundle events to all registered bundle listeners.
     * This replaces the AspectJ BundleEvents aspect functionality.
     * 
     * @param eventType The type of bundle event to fire
     */
    private void fireBundleEvent(int eventType) {
        StubBundleContext context = this.bundleContext;
        if (context != null) {
            List<BundleListener> listeners = context.getBundleListeners();
            BundleEvent event = new BundleEvent(eventType, this);
    ...
    /**
     * Helper method to check if bundle is not uninstalled.
     * This replaces the AspectJ BundleValid aspect functionality.
     * 
     * @throws IllegalStateException if the bundle has been uninstalled
     */
    private void ensureNotUninstalled() {
        if (this.state == Bundle.UNINSTALLED) {
