...
class Candidates
{
    ...
    ResolutionError checkSubstitutes()
    {
        ...
        for (Map.Entry<Capability, Integer> substituteStatus : substituteStatuses.fast())
        {
			...
			m_session.permutateIfNeeded(PermutationType.SUBSTITUTE, substitutedReq, copy);
                        ...
                        while (!candidates.isEmpty())
                        {
                            ...
                            switch (candidateStatus)
                            {
                                ...
                                default:
									...
									if (candidates.getRemainingCandidateCount() > 1) {
										// FIXME this removes valid candidates after the permutation!
    ...
    {
		System.out.println("Candidates.removeFirstCandidate: " + req);
        CandidateSelector candidates = m_candidateMap.get(req);
        // Remove the conflicting candidate.
        Capability cap = candidates.removeCurrentCandidate();
		System.out.println("\t-> " + cap);
		System.out.println("\tEmpty " + candidates.isEmpty());
        if (candidates.isEmpty())
        {
